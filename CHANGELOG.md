...
## [2026-03-08] — 選書の実在検証厳格化（楽天API必須化）

### ✨ Feature (選書クオリティ維持 + 架空本完全排除)
- **楽天ブックスAPI必須検証**: 表示する全書籍は楽天ブックスAPIでの実在確認+表紙画像取得が必須。架空の本が表示される問題を根絶
- **AI候補数を9冊に拡大**: Gemini 2.5 Flash（選書クオリティ最優先）に9冊候補を要求し、楽天API検証をパスした上位3冊のみ表示
- **Google Books/openBDフォールバック廃止**: 楽天のみで一元的に実在判定（シンプルかつ確実）
- **並列API検証**: Promise.allで全9候補を同時に楽天APIへ問い合わせ、速度を最大化
- **AIの推薦順序維持**: 検証は並列、採用はAIの推薦順（クオリティ順）に上位から選出
- **Amazonリンク維持**: 楽天で検証した本にもAmazon購入リンクを併記
- **AIプロンプト強化**: 「楽天ブックスに掲載されている書籍を優先」指示を追加
