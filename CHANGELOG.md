...
### 🔧 UX改善
- TOPページの「わたしの本棚」ボタンをログイン状態に関わらず常時表示するように変更。ログアウト後でも/libraryのログイン画面に直接遷移可能に
...
### 🔧 Config
- **OGP画像を1200x630に再生成**: 旧画像が640x640（正方形）でSNSシェア時に不適切だったため、標準サイズの1200x630で再生成。背景グラデーション＋テキストのみのクリーンなデザイン
- **metadataBase修正**: フォールバックURLを旧`compass-app.vercel.app`から`compass.hitokoto.tech`に更新

## [2026-03-10] — 選書タイムアウト修正 + インシデントレポート未修正問題3件の解消
...
### 🔧 インシデントレポート #59-61 未修正問題の解消
- **AI ISBNの完全除去**: システムプロンプト・出力JSONスキーマ・ユーザープロンプトからISBN指示を完全削除。AIのISBNハルシネーションによる表紙不一致リスクを根絶（インシデント#1,#19,#20の教訓）
- **フラグメント二重括弧+全角スペース修正**: フロント側で先頭末尾の括弧を除去してから単一`「」`で囲むロジックに変更。AIプロンプトにも「括弧で囲まず素のテキストで出力」指示を追加
- **古いコメント更新**: 「Google Books カスケード」という実態と異なるコメントを「楽天タイトル+著者検索 + titleLooseMatchガード」に修正
