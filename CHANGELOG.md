...
## [2026-03-08] — 選書アーキテクチャ全面再設計（楽天API厳格検証+高速化）

### ✨ Architecture Overhaul (架空本完全排除 + 速度最適化)
- **楽天ブックスAPI必須検証**: 表示する全書籍は楽天APIでの実在確認+表紙画像取得が必須
- **AI候補9冊→楽天検証→通過3冊のみ表示**: Gemini 2.5 Flash（クオリティ維持）に9冊候補を要求
- **Progressive Verification Architecture**: 
  - バッチ1: AI選書(~10s) + 楽天逐次検証(~3-8s) → 3冊表示 + 残り未検証候補をフロントに返却
  - バッチ2/3: **AIコール完全スキップ** → 残り候補の楽天検証のみ(~3-5s)で超高速
- **楽天APIレートリミット対応**: 1req/sec制限に対し逐次検証+1.1秒間隔で安全に実行
- **Google Books/openBDフォールバック廃止**: 楽天のみで一元的に実在判定
