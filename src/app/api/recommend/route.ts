...
/**
 * 表紙画像URL取得 — ISBNベースの3段階フォールバック:
 *   1. openBD カバー (cover.openbd.jp/{isbn}.jpg) — 主軸。日本語書籍はほぼ100%カバー
 *   2. Google Books API サムネイル — openBDに画像がない場合のフォールバック
 *   3. 空文字 → フロントエンドでCSSプレースホルダー
 */
async function getBookCover(isbn: string, title: string, author: string): Promise<string> {
  // ── Stage 1: openBD (ISBN → カバー画像。主軸) ──
  if (isbn && /^\d{13}$/.test(isbn)) {
    // openBDのカバー画像はリダイレクトで画像を返す。
    // HEADリクエストで存在確認し、200なら使う
    try {
      const openbdUrl = `https://cover.openbd.jp/${isbn}.jpg`;
      const headRes = await fetch(openbdUrl, {
        method: 'HEAD',
        signal: AbortSignal.timeout(3000),
      });
      if (headRes.ok && headRes.headers.get('content-type')?.includes('image')) {
        console.log(`[Cover] openBD hit for "${title}": ${openbdUrl}`);
        return openbdUrl;
      }
      console.log(`[Cover] openBD miss for "${title}" (ISBN: ${isbn})`);
    } catch (e) {
      console.warn(`[Cover] openBD check failed for "${title}":`, e);
    }
  }

  // ── Stage 2: Google Books API サムネイル ──
  const googleApiKey = process.env.GOOGLE_BOOKS_API_KEY || '';
  if (googleApiKey) {
    try {
      const query = encodeURIComponent(`${title} ${author}`);
      const res = await fetch(
        `https://www.googleapis.com/books/v1/volumes?q=${query}&langRestrict=ja&maxResults=3&fields=items(volumeInfo(title,imageLinks))&key=${googleApiKey}`,
          ...
          for (const item of items) {
            ...
            if (rawUrl) {
              const url = rawUrl
                .replace('http://', 'https://')
                .replace('&edge=curl', '');
              console.log(`[Cover] GoogleBooks fallback for "${title}": ${url}`);
              return url;
            }
          }
        }
  ...
  // ── Stage 3: ISBNがあるならopenBDを信じて返す（HEADチェック失敗でもブラウザで表示できるケースあり） ──
  if (isbn && /^\d{13}$/.test(isbn)) {
    const fallbackUrl = `https://cover.openbd.jp/${isbn}.jpg`;
    console.log(`[Cover] openBD direct fallback for "${title}": ${fallbackUrl}`);
    return fallbackUrl;
  }

  console.log(`[Cover] No cover for "${title}"`);
  return '';
}
...
出力は上記で指定されたJSON形式のみを、コードブロック(\`\`\`json ... \`\`\`)で囲んで出力してください。JSON以外のテキストは一切出力しないでください。`;
    ...
    // Phase 2: 表紙画像取得（openBD主軸 → Google Booksフォールバック）
