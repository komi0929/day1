...
3. **${AI_REQUEST_COUNT}冊すべてが異なる切り口**：同じジャンル・同じ著者に偏らない
...
6. **楽天ブックスに掲載されている書籍を優先すること**：楽天ブックスで検索してヒットする書籍を中心に選ぶ。絶版や電子書籍のみの書籍は避ける
...
\`\`\`json
{
  ...
  "fragments": ["note本文から印象的な一節を5〜8つ抽出。各20〜60字程度。括弧や記号で囲まない素のテキストで出力すること"]
}
\`\`\``;
}
...
${wantFragments ? '- fragmentsはnote本文から印象的な一節を5〜8つ抽出する。括弧や記号で囲まず、素のテキストのみで出力すること。全角スペースは使わないこと' : '- fragmentsは空配列[]にする'}
...
指定されたJSON形式のみ出力してください。`;
    ...
    // 逐次検証（楽天タイトル+著者検索 + titleLooseMatchガード）
