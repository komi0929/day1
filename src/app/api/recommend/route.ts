...
## 推薦の鉄則
1. **実在する書籍のみ推薦する（最重要）**：架空の本は絶対に推薦しない。Amazonや書店で購入できる実在の書籍のみ。タイトルと著者名は一字一句正確に。「それっぽいタイトル」を作らないこと。
...
function isTitleMatch(requested: string, found: string): boolean {
  ...
  // タイトルの主要部分（最初の4文字以上）が含まれているか
  const minLen = Math.min(r.length, f.length);
  if (minLen >= 4) {
    const shortKey = r.length <= f.length ? r.substring(0, Math.max(4, Math.floor(r.length * 0.6))) : f.substring(0, Math.max(4, Math.floor(f.length * 0.6)));
    if (r.includes(shortKey) && f.includes(shortKey)) return true;
  }
  return false;
}
 ...
 *
 * Returns: { thumbnail, verified }
 *   - verified=true: Google Booksでタイトルが一致する書籍が見つかった（実在確認済み）
 *   - verified=false: 見つからなかった（架空の可能性）
 */
async function searchBookCover(title: string, author: string): Promise<{ thumbnail: string; verified: boolean }> {
      ...
      if (res.ok) {
        ...
        if (items && items.length > 0) {
          for (const item of items) {
            ...
            // タイトルが一致しない結果はスキップ
            if (!isTitleMatch(title, foundTitle)) {
              continue;
            }
            ...
            if (rawUrl) {
              const url = rawUrl
                ...
                .replace('&edge=curl', '');
              console.log(`[Cover] GoogleBooks match for "${title}" (found: "${foundTitle}"): ${url}`);
              return { thumbnail: url, verified: true };
            }
  ...
  if (isbn13) {
    ...
    return { thumbnail: openbdUrl, verified };
  }

  console.log(`[Cover] No cover for: "${title}" (verified=${verified})`);
  return { thumbnail: '', verified };
}
...
出力は上記で指定されたJSON形式のみを、コードブロック(\`\`\`json ... \`\`\`)で囲んで出力してください。JSON以外のテキストは一切出力しないでください。`;
    ...
    // Phase 2: Google Booksで表紙取得 + 実在検証
    const coverResults = await Promise.all(
      books.map(async (book) => {
        const result = await searchBookCover(book.title, book.author);
        return { book, ...result };
      })
    );

    // 実在が確認できなかった書籍は除外
    const enrichedBooks: BookResult[] = coverResults
      .filter(({ verified, book }) => {
        if (!verified) {
          console.warn(`[Filter] Excluded unverified book: "${book.title}" by ${book.author}`);
        }
        return verified;
      })
      .map(({ book, thumbnail }) => {
